			...
			<form id="form1" name="form1"
				...
				<div class="sch">
					...
					<p>
						...
						<select id="brandId" name="brandId" value="">
							...
							<c:forEach items="${bList }" var="brand">
								<option value="${brand.brandId }"
									<c:if test="${brandId == brand.brandId}">selected</c:if>>${brand.brandName }
								</option>
							</c:forEach>
						</select> 
						<select id="auditStatus" name="auditStatus">
							...
							<option value="0"
								<c:if test="${auditStatus == 0}">selected</c:if>>待审核</option>
							<option value="1"
								<c:if test="${auditStatus == 1}">selected</c:if>>通过</option>
							<option value="2"
								<c:if test="${auditStatus == 2}">selected</c:if>>不通过</option>
						</select> 
				...
				<div class="page_c">
					<span class="l"> </span> <span class="r inb_a"> <a
						href="${path}/shop/item/addItem.jsp" class="btn80x20" title="添加商品">添加商品</a>
					</span>
				</div>
				...
				<div class="page_c">
					...
					<input type="hidden" value="3" id="pageNo" name="pageNo" />
					<input 	type="hidden" value="${page.totalCount}" id="totalCount" 	name="totalCount" /> 
					<input type="hidden" value="${page.totalPage}" id="totalPage" name="totalPage" /> 共
						<var	id="pagePiece" class="orange">0</var>条<var id="pageTotal">1/1</var>
						<a href="javascript:void(0);" id="previous" class="hidden"
						title="上一页">上一页</a> <a href="javascript:void(0);" id="next"
						class="hidden" title="下一页">下一页</a>
					</span>
				</div>
			</form>
		</div>
